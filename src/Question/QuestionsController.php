...
class QuestionsController implements \Anax\DI\IInjectionAware
{
    ...
    public function commentAction($type = null, $id = null, $q_id = null)
    {
        ...
        if (!$this->auth->isAuthenticated()) {
            $this->FlashMessages->add('error', 'Please sign in!');
        ...
        if ($status === true) {
            ...
            $this->FlashMessages->add('success', "Your comment is published.");
    ...
    public function newAction()
    {
        ...
        if (!isset($acronym)) {
            ...
            $form = $form->create([], [
                ...
                'tags' => [
                    'type'        => 'text',
                    'placeholder' => 'Arrow, Smallville, Characters',
                    'required'    => false,
                ],
                'submit' => [
                    'type'      => 'submit',
            ...
            if ($status === true) {
                ...
                if (isset($tags)) {
                    ...
                    if (sizeof($tags) > 8) {
                        ...
                        $this->FlashMessages->add('error', "You can not use more than <b>8</b> tags!");
                        $this->response->redirect($_SERVER['PHP_SELF']);
                        exit();
                    }
                    foreach ($tags as $tag => $value) {
                        if (strlen($value) > 16) {
                            $this->FlashMessages->add('error', "Tags can not be longer than <b>16</b> characters!");
                            $this->response->redirect($_SERVER['PHP_SELF']);
                            exit();
                        }
                        if (empty($value)) {
                ...
                $this->FlashMessages->add('success', "Your question is published.");
    ...
    public function voteAction($id = null, $value = null)
    {
        ...
        if (!$this->auth->isAuthenticated()) {
            $this->FlashMessages->add('error', 'Please sign in to vote!');
        ...
        if (is_bool($votes) || !in_array($id, $votes)) {
            ...
            $this->FlashMessages->add('warning', 'You have already voted on this question!');
        }
    ...
    public function voteAnswerAction($id = null, $value = null)
    {
        ...
        if (is_bool($votes) || !in_array($id, $votes)) {
            ...
            $this->FlashMessages->add('warning', 'You have already voted on this answer!');
        }
    ...
    public function correctAction($id = null, $answer_id = null)
    {
        ...
        if (!$this->auth->isAuthenticated()) {
            $this->FlashMessages->add('error', 'You do not have permission to this page!');
            $this->response->redirect($_SERVER['HTTP_REFERER']);
            exit();
        }
        ...
        if ($question->correct_answer == $answer_id) {
            ...
            $this->FlashMessages->add('success', 'You have unmarked a correct answer!');
        } else {
            $question->correct_answer = $answer_id;
            $this->FlashMessages->add('success', 'You have marked a correct answer!');
        }
    ...
    public function answerAction($id = null, $slug = null)
    {
        ...
        if ($status === true) {
            ...
            $this->FlashMessages->add('success', "Your answer is published.");
    ...
    public function setupAction() {
        ...
        $this->views->addString('<h3>Question & Answer database was successfully setup!</h3>', 'main');
    }
}
