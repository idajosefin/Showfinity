...
class UsersController implements \Anax\DI\IInjectionAware
{
    ...
    public function logoutAction()
    {
        ...
        $this->auth->logout();
        $this->FlashMessages->add('info', "See you later {$user->username}!");
    ...
    public function loginAction()
    {
        if ($this->auth->isAuthenticated()) {
            $this->response->redirect($this->url->create(''));
            $this->FlashMessages->info('You are already logged in!');
            exit();
        }
        ...
        if ($status === true) {
            ...
            if ($this->auth->authenticate($acronym, $password)) {
                ...
                $this->FlashMessages->add('success', 'Welcome!');
                $url = $this->url->create('');
            } else {
                $this->FlashMessages->add('error', 'Incorrect username or password!');
    ...
    public function registerAction()
    {
        ...
        if ($this->auth->isAuthenticated()) {
            $this->FlashMessages->add('warning', 'You are already signed in!');
            $url = $this->url->create('');
            $this->response->redirect($url);
            exit();
        }
        ...
        if (!isset($acronym)) {
            ...
            if ($status === true) {
                ...
                if (strlen($password) < 6) {
                    ...
                    $this->FlashMessages->add('error', "Your password must be atleast <b>6</b> characters long!");
                    $url = $this->url->create('users/register');
                    $this->response->redirect($url);
                    exit();
                }
                ...
                if (strlen($acronym) > 16) {
                    ...
                    $this->FlashMessages->add('error', "Your username can not be longer than <b>16</b> characters!");
                    $url = $this->url->create('users/register');
                    $this->response->redirect($url);
                    exit();
                }
                ...
                $this->FlashMessages->add('success', "Welcome {$name}!\nYour account was successfully registered!");
    ...
    public function editAction($username)
    {
        ...
        if (isset($username)) {
            ...
            if ($status === true) {
                ...
                $this->FlashMessages->add('success', "Your profile was successfully updated!");
    ...
    public function deleteAction($id = null)
    {
        ...
        if (!isset($id)) {
            ...
            $this->FlashMessages->add('success', 'User deleted!');
    ...
    public function setupAction() {
        ...
        $this->users->save([
            'acronym' => 'ider',
            'email' => 'info@idajosefin.com',
            'name' => 'Ida Eriksson',
            'password' => password_hash("ider123", PASSWORD_DEFAULT),
            'permission' => '3',
            'created' => $now,
            'active' => $now,
        ]);

        $this->views->addString('<h3>User database was successfully setup!</h3>', 'main');
    }
}
